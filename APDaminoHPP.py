...
labels = []
genes = []
with open(protocol + "NormalizationTE.csv", "r") as file:
    data = file.readlines()
    for i in data:
        line = i.strip().split(',')
        temp = []
...
plt.boxplot(six_apd, labels = six_labels)
plt.title("Normalized Average Pairwise Distance Within Amino Acids with 6 Codons (" + protocol + " Expression)\nSample Rate = " + str(sample_rate))
